namespace Algorithms
{
    class Tools
    {
        ...
        {
            if(pathToText != null){string _text = File.ReadAllText(pathToText);
            ...
            if(pathToTextHashTables != null)
            {
                ...
                for (int i = 0; i < InsertHashTables.Length; i++) {InsertHashTables[i] = InsertHashTables[i].Trim(trimChars);}
                for(int i = 0; i < InsertHashTables.Length / 10; i++){SearchHashTables[i] = InsertHashTables[rand.Next(0, InsertHashTables.Length)];}
